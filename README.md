...
Global client-side git hooks, initially inspired by [caulking](https://github.com/cloud-gov/caulking/).
...
- Try to install [talisman](https://github.com/thoughtworks/talisman). You have to answer a few prompts:
  ...
  - When the installation asks where to add the PATH information, choose `.profile` (or "I will set it later" then follow the instructions)


## What does it do?

- Provides a way of setting up git hooks that work globally, across all your repos
- Uses that pattern to run a pre-commit check using talisman, which scans all your staged changes for potential credential leaks
...
- See the [docs](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) for more information about git hooks
